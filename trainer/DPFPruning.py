...
class DPFPruning(tf.keras.callbacks.Callback):
    ...
    def __init__(self, weight_predicate=None,
        ...
        assert start_pruning_at_epoch <= finish_pruning_by_epoch # type: ignore
        super().__init__()
        ...
        self.t_n = finish_pruning_by_epoch - 1 # type: ignore
    ...
    def on_train_begin(self, logs=None):
        ...
        self.weights = [w for w in self.model.trainable_weights if self.filter(w, self.model)] # type: ignore
    ...
    def on_train_batch_end(self, batch, logs=None):
        self.iterations += 1
        logs["sparsity"] = self.s_t # type: ignore
