...
from tensorflow.python.ops import gen_audio_ops as audio_ops # type: ignore
from tensorflow.python.util import compat # type: ignore
import hashlib
import math
import os.path
import random
...
class AudioProcessor(object):
    ...
    def maybe_download_and_extract_dataset(self, data_url, dest_directory):
        ...
        if not tf.io.gfile.exists(filepath):
            ...
            try:
                filepath, _ = urllib.request.urlretrieve(data_url, filepath, _progress) # type: ignore
            except:
    ...
    def get_dataset(self, background_frequency, background_volume_range, time_shift, mode):
        ...
        def preprocess(audio, label):
            ...
            if use_background or label == SILENCE_INDEX:
                background_index = tf.random.uniform([], 0, self.background_data.shape[0], dtype=tf.int32) # type: ignore
                background_samples = self.background_data[background_index] # type: ignore
...
class SpeechDataset:
    ...
    def load_sample(self, file, foreground_volume=1.0, sample_rate=None):
        return self.audio_processor.load_sample(file, foreground_volume, sample_rate) # type: ignore


class SpeechCommands(Dataset):
    def __init__(self, data_dir, words=None,rate=30,reso=30,chan=40):
        self.words = words or DEFAULT_WORDS
        # change here
        self.rate = rate
        self.reso = reso
        self.chan = chan
        print('rate, reso, chan in init_dataset: ',self.rate, self.reso, self.chan)
        self.loader = SpeechDataset(words=self.words, data_dir=data_dir,window_stride=self.rate, feature_bin_count=self.chan, window_size_ms=self.reso)
        self.data_dir = data_dir
    ...
    def update_dataset(self, model_list):
        print('model list in update_dataset: ',model_list)
        sam_list = model_list['sample']
        self.rate = sam_list[0]
        self.reso = sam_list[1]
        self.chan = sam_list[2]
        print('rate, reso, chan in update_dataset: ',self.rate, self.reso, self.chan)
        self.loader = SpeechDataset(words=self.words, data_dir="datasets/speech_commands_v0.02",window_stride=self.rate, feature_bin_count=self.chan, window_size_ms=self.reso)
        print("train shape in update dataset ",self.loader.sample_shape())
        self.data_dir ="/datasets/speech_commands_v0.02"
